...
export async function GET() {
  try {
    ...
    const [tasks, appointments, cases, activities] = await Promise.all([
      supabase.from("tasks").select("id,case_id,client_id,property_id,title,description,category,action_type,priority_score,due_at,due_on,due_context,status").eq("user_id", user.id).eq("status", "pending").order("priority_score", { ascending: false }).order("due_at", { ascending: true, nullsFirst: false }).limit(50),
      ...
      supabase.from("activity_events").select("id,case_id,client_id,event_type,title,details,created_at").eq("user_id", user.id).order("created_at", { ascending: false }).limit(8),
    ]);
    const queryError = tasks.error || appointments.error || cases.error || activities.error;
    ...
    return NextResponse.json({
      ...
      counts: {
        ...
        overdue: rows.filter((task) => (task.due_at && task.due_at < now.toISOString()) || (task.due_on && task.due_on < today)).length,
      },
    });
  } catch (error) {
