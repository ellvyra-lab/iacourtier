...
export default async function TaskPage({ params }: { params: { id: string } }) {
  ...
  return <article className="mx-auto max-w-2xl space-y-5 rounded-3xl border border-slate-200 p-6">
    ...
    <p className="whitespace-pre-wrap">{task.description}</p><p>Échéance : {task.due_on || (task.due_at ? new Date(task.due_at).toLocaleString("fr-CA", { timeZone: "America/Toronto" }) : "non précisée")}</p>
