...
import { ArrowRight, CalendarDays, FileText, Home, Loader2, Mail, Phone, Sparkles } from "lucide-react";
...
export function Client360Workspace({ clientId }: { clientId: string }) {
  ...
  return <div className="space-y-6">
    <nav className="text-sm text-slate-500"><Link href="/tableau-de-bord/clients" className="hover:text-teal-700">Clients & dossiers</Link><span className="mx-2">›</span><span>{name}</span></nav>
    ...
    <div className="grid gap-6 xl:grid-cols-2">
      ...
      <Section title="Propriétés" empty="Aucune propriété reliée.">{data.properties.map((item, index) => { const property = Array.isArray(item.property) ? item.property[0] : item.property; return <div key={`${property?.id || index}-${item.relationship}`} className="rounded-2xl border border-slate-200 p-4 dark:border-slate-800"><strong>{property?.address || "Adresse à confirmer"}</strong><p className="mt-1 text-sm text-slate-500">{[property?.city, property?.postal_code, property?.property_type].filter(Boolean).join(" · ")} · {label(item.relationship)}</p></div>; })}</Section>
      ...
      <Section title="Historique" empty="Aucun événement.">{data.activity.map((item) => <Timeline key={item.id} icon={<span className="h-2 w-2 rounded-full bg-teal-600" />} title={item.title} date={item.created_at} detail={item.details} />)}</Section>
    </div>
  </div>;
}
