...
export function UniversalSearch({ mobile = false }: { mobile?: boolean }) {
  ...
  useEffect(() => {
    if (authStatus !== "authenticated" || query.trim().length < 2) { setResults([]); return; }
    const controller = new AbortController();
    const timeout = window.setTimeout(async () => {
      setLoading(true);
      try {
        const response = await authenticatedFetch(`/api/clients?q=${encodeURIComponent(query.trim())}`, { signal: controller.signal, cache: "no-store" });
  ...
  return <div className={"relative w-full max-w-xl " + (mobile ? "block" : "hidden md:block")}><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Rechercher un client, une adresse ou un dossier" className="min-h-10 w-full rounded-xl border border-subtle bg-surface-soft pl-10 pr-10 text-sm" />{loading ? <Loader2 className="absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 animate-spin text-teal-700" /> : null}
