  ...
  return (
    <form onSubmit={save} className="mx-auto max-w-6xl space-y-6">
      ...
      <section className="rounded-2xl border border-subtle bg-surface p-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-wide text-muted">Intégration</p><h2 className="mt-1 text-xl font-semibold">Centris</h2><p className="mt-2 max-w-2xl text-sm text-muted">IACourtier peut préparer, vérifier, copier et exporter les données d’une inscription. Aucune transmission directe n’est activée sans connexion officielle autorisée.</p></div><span className="rounded-full bg-amber-100 px-3 py-1 text-sm font-bold text-amber-900">Non connectée</span></div>
        <p className="mt-4 rounded-xl bg-amber-50 p-3 text-sm text-amber-900">La transmission directe sera disponible lorsqu’une connexion Centris autorisée sera configurée.</p>
      </section>
