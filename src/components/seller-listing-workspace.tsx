...
export function SellerListingWorkspace({ id }: { id: string }) {
  ...
  return <div className="space-y-7">
    <div className="flex flex-wrap gap-4 text-sm font-semibold"><Link href="/tableau-de-bord/clients" className="text-slate-600 dark:text-slate-300">← Clients & dossiers</Link>{data.listing.client_case_id ? <Link href={`/tableau-de-bord/dossiers/${data.listing.client_case_id}`} className="text-teal-700">Voir le dossier unifié →</Link> : null}<Link href={`/tableau-de-bord/inscriptions/${id}/centris`} className="text-teal-700">Préparer pour Centris →</Link></div>
    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
      ...
      <nav className="flex flex-wrap gap-2 border-t border-slate-200 px-4 py-3 text-sm font-semibold dark:border-slate-800">{[["dossier","Résumé"],["dossier","Client"],["dossier","Propriété"],["dossier","Documents"],["suivis","Étapes"],["marketing","Marketing"],["suivis","Communications"],["suivis","Transaction"]].map(([href,label]) => <a key={label} href={`#${href}`} className="rounded-full px-3 py-2 hover:bg-slate-100 dark:hover:bg-slate-800">{label}</a>)}<Link href={`/tableau-de-bord/inscriptions/${id}/centris`} className="rounded-full px-3 py-2 text-teal-700 hover:bg-teal-50">Centris</Link></nav>
    </section>
