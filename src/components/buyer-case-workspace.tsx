...
export function BuyerCaseWorkspace({ id }: { id: string }) {
  ...
  return <div className="space-y-7">
    ...
    <section id="criteres" className="scroll-mt-6"><Panel title="Qualification, financement et critères" icon={WalletCards}><Definition items={[["Budget",data.case.budget],["Préapprobation",labelPreapproval(data.case.preapproval_status)],["Secteurs",data.case.sectors.join(", ")],["Type",data.case.property_type],["Chambres",data.case.bedrooms],["Besoins",data.case.important_needs],["Échéancier",data.case.timeline],["Propriété à vendre",data.case.property_to_sell == null ? null : data.case.property_to_sell ? "Oui" : "Non"]]} /></Panel></section>
    <section id="documents" className="scroll-mt-6"><Panel title="Documents" icon={UploadCloud}><label className="inline-flex cursor-pointer rounded-xl bg-slate-950 px-4 py-3 text-sm font-semibold text-white dark:bg-white dark:text-slate-950">{busy === "documents" ? "Téléversement…" : "Ajouter des documents"}<input type="file" multiple accept=".pdf,.jpg,.jpeg,.png,.heic,.heif,.webp" className="hidden" onChange={(event) => upload(event.target.files)} /></label>{data.documents.length ? <div className="grid gap-2 sm:grid-cols-2">{data.documents.map((document) => <div key={document.id} className="rounded-xl border border-slate-200 p-3 dark:border-slate-700"><p className="font-semibold">{document.name}</p><p className="text-xs text-slate-500">{document.document_type}</p></div>)}</div> : <p className="text-sm text-slate-500">Aucun document sauvegardé.</p>}</Panel></section>
