...
type BuyerData = {
  case: {
    ...
    contact: QuickClient | QuickClient[];
  };
...
export function BuyerCaseWorkspace({ id }: { id: string }) {
  ...
  return <div className="space-y-7">
    ...
    <section id="client" className="scroll-mt-6"><Panel title="Client" icon={UserRound}>{contact ? <ClientQuickPanel client={contact} caseId={data.case.client_case_id} caseLabel={`Dossier acheteur · ${name}`} returnHref={`/tableau-de-bord/acheteurs/${id}`} returnLabel="au dossier acheteur" onUpdated={() => { void load(); }} /> : <p className="text-sm text-slate-500">Client à identifier.</p>}</Panel></section>
