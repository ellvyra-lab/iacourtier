...
export function CrmCallButton({ clientId, caseId = null, propertyId = null, taskId = null, phone, clientName, compact = false, onCompleted }: {
  ...
  async function saveResult() {
    ...
    try {
      const response = await authenticatedFetch(`/api/crm/calls/${callId}/result`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, nextContactAt: form.nextContactAt ? new Date(form.nextContactAt).toISOString() : null }) });
  ...
  if (!normalized) return <div className="inline-flex flex-col items-start gap-1"><button type="button" disabled className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-slate-200 px-3 text-sm font-semibold text-slate-400 dark:border-slate-700"><PhoneOff className="h-4 w-4" />Téléphone manquant</button><div className="flex flex-wrap gap-x-3 gap-y-1 text-xs font-semibold"><Link href={`/tableau-de-bord/clients/${clientId}`} className="text-amber-700">Ajouter ou modifier le numéro</Link><Link href={`/tableau-de-bord/importer?clientId=${clientId}&source=document`} className="text-teal-700">Rechercher dans un document</Link><Link href={`/tableau-de-bord/importer?clientId=${clientId}&source=image`} className="text-teal-700">Importer une capture</Link></div></div>;
  ...
  return <>
    <div className="inline-flex flex-wrap items-center gap-2">
      ...
      {callId ? <button type="button" onClick={() => setOpen(true)} className="min-h-10 rounded-xl border border-emerald-600 px-3 text-sm font-semibold text-emerald-700">Enregistrer le résultat de l’appel</button> : null}
