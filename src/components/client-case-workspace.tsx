...
type Payload = {
  ...
  clients: Array<QuickClient & Record<string, any>>;
...
export function ClientCaseWorkspace({ caseId, importCompleted = false, addDocument = false }: { caseId: string; importCompleted?: boolean; addDocument?: boolean }) {
  ...
  useEffect(() => { if (data && window.location.hash) document.getElementById(decodeURIComponent(window.location.hash.slice(1)))?.scrollIntoView({ block: "center" }); }, [data]);
  ...
  return <div className="space-y-6">
    ...
    <div id="parcours" className="grid gap-6 xl:grid-cols-2">
      <Panel title="Client et propriété" empty="Aucun client relié.">{data.clients.map((client) => <ClientQuickPanel key={client.id} client={client} caseId={caseId} caseLabel={item.title} returnHref={`/tableau-de-bord/dossiers/${caseId}`} returnLabel="au dossier" onUpdated={() => { void load(); }} />)}{item.property ? <PropertyQuickCard property={property(item.property)} caseId={caseId} returnHref={`/tableau-de-bord/dossiers/${caseId}`} returnLabel="au dossier" specializedHref={specializedHref} /> : null}</Panel>
      ...
      <Panel title="Tâches" empty="Aucune tâche.">{data.tasks.map((task) => <button key={task.id} id={`task-${task.id}`} type="button" disabled={savingId === task.id} onClick={() => update("task", task.id, task.status === "completed" ? "pending" : "completed")} className="flex w-full items-center gap-3 rounded-xl border border-slate-200 p-3 text-left dark:border-slate-800">{savingId === task.id ? <Loader2 className="h-5 w-5 animate-spin" /> : task.status === "completed" ? <CheckCircle2 className="h-5 w-5 text-teal-700" /> : <Circle className="h-5 w-5 text-slate-400" />}<span><strong className={task.status === "completed" ? "line-through opacity-60" : ""}>{task.title}</strong><span className="block text-xs text-slate-500">{label(task.category)}{task.due_on ? ` · ${task.due_on}` : task.due_at ? ` · ${date(task.due_at)}` : ""}</span></span></button>)}</Panel>
      <Panel title="Documents" empty="Aucun document classé.">{data.documents.map((document) => <div key={document.id} id={`document-${document.id}`} className="flex items-center gap-3 rounded-xl border border-slate-200 p-3 dark:border-slate-800">{document.is_sensitive ? <LockKeyhole className="h-5 w-5 text-amber-700" /> : <FileText className="h-5 w-5 text-teal-700" />}<span><strong className="block text-sm">{document.name}</strong><span className="text-xs text-slate-500">{document.category} · {label(document.analysis_status)}{document.is_sensitive ? " · accès restreint" : ""}</span></span></div>)}</Panel>
