...
export function ClientCaseWorkspace({ caseId, importCompleted = false, addDocument = false }: { caseId: string; importCompleted?: boolean; addDocument?: boolean }) {
  ...
  return <div className="space-y-6">
    ...
    {importCompleted ? <section className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-emerald-950 dark:border-emerald-900 dark:bg-emerald-950/20 dark:text-emerald-100" role="status"><div className="flex items-start gap-3"><CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0 text-emerald-700" /><div><strong>Import terminé — dossier prêt à {item.progress || 0} %</strong><p className="mt-1 text-sm">{data.documents.length} document{data.documents.length > 1 ? "s" : ""} classé{data.documents.length > 1 ? "s" : ""}, {data.clients.length} client{data.clients.length > 1 ? "s" : ""} relié{data.clients.length > 1 ? "s" : ""}{reviewCount ? ` et ${reviewCount} élément${reviewCount > 1 ? "s" : ""} non bloquant${reviewCount > 1 ? "s" : ""} à vérifier` : " et aucune ambiguïté bloquante"}.</p></div></div></section> : null}
    <header className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between"><div><p className="text-sm font-semibold text-teal-700">Dossier {caseType(item.case_type)}</p><h1 className="mt-2 text-3xl font-semibold">{item.title}</h1><p className="mt-2 text-slate-500">Étape actuelle : <strong className="text-slate-900 dark:text-white">{label(item.pipeline_stage)}</strong></p></div><div className="min-w-64 rounded-2xl bg-teal-50 p-4 text-teal-950 dark:bg-teal-950 dark:text-teal-50"><p className="text-xs font-bold uppercase tracking-wide">Continuer intelligemment</p><p className="mt-2 font-semibold">{continueLabel}</p><a href={continueHref} className="mt-3 inline-flex items-center gap-2 text-sm font-bold text-teal-700 dark:text-teal-300">Continuer <ArrowRight className="h-4 w-4" /></a></div></div>
    ...
    <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      <Metric title="Clients" value={data.clients.length} /><Metric title="Documents" value={data.documents.length} /><Metric title="Tâches ouvertes" value={data.tasks.filter((task) => task.status === "pending").length} /><Metric title="À vérifier" value={reviewCount} />
    </section>
