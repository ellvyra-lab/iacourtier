...
import type { CoachMessage, CoachReply } from "@/lib/coach/conversation";

type ConversationState = { messages: CoachMessage[]; busy: boolean; error: string; send: (text: string, choiceId?: string, taskId?: string) => Promise<void>; reset: () => Promise<void>; open: boolean; setOpen: (value: boolean) => void; voice: number; consumeVoice: () => void; launch: (voice?: boolean) => void };
...
export function CoachConversationProvider({ children }: { children: ReactNode }) {
  ...
  async function send(text: string, choiceId?: string, taskId?: string) {
    ...
    try {
      ...
      const response = await authenticatedFetch("/api/coach/conversation", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ conversationId, messageId, text, choiceId, taskId }) });
      const body = await response.json();
...
export function CoachConversation({ panel = false }: { panel?: boolean }) {
  ...
  return <section ref={container} style={availableHeight ? { height: availableHeight } : undefined} className={`mx-auto flex w-full max-w-4xl flex-col ${panel ? "h-full" : "h-[calc(100dvh-10rem)]"}`} aria-label="Conversation avec le Coach IA" onKeyDown={event => { if (panel && event.key === "Escape") setOpen(false); }}>
    ...
    <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-3 py-6" role="log" aria-live="polite" aria-relevant="additions">
      {!messages.length ? <div className="mx-auto max-w-xl py-8"><p className="text-2xl font-semibold">Bonjour{name ? ` ${name}` : ""}.</p><p className="mt-2 text-xl text-slate-500">Qu’est-ce qu’on fait aujourd’hui ?</p><div className="mt-8 grid gap-3 sm:grid-cols-2">{["Qu’est-ce qui presse aujourd’hui ?", "Montre-moi mes suivis.", "Trouve un client.", "Ajoute une tâche."].map(suggestion => <button type="button" disabled={busy} key={suggestion} onClick={() => { if (suggestion === "Trouve un client." || suggestion === "Ajoute une tâche.") setText(suggestion === "Trouve un client." ? "Trouve " : "Rappelle-moi de "); else void submit(suggestion); }} className="min-h-14 rounded-2xl border border-slate-200 p-4 text-left text-sm hover:border-blue-400 dark:border-slate-800">{suggestion}</button>)}</div></div> : messages.map((message, index) => <div key={message.id} className="mb-6 space-y-4"><div className="ml-auto max-w-[88%] whitespace-pre-wrap break-words rounded-2xl bg-blue-600 px-4 py-3 text-white">{message.text}</div>{message.reply ? <div className="max-w-full space-y-3"><p className="whitespace-pre-wrap break-words leading-relaxed">{message.reply.text}</p>{message.reply.cards.map(card => <div key={`${card.kind}-${card.id}`} className="flex items-center justify-between gap-3 rounded-2xl border border-slate-200 p-4 dark:border-slate-800"><div className="min-w-0"><Link href={card.href} className="break-words font-semibold text-blue-700">{card.title}<ArrowUpRight className="ml-1 inline h-4 w-4" /></Link>{card.detail ? <p className="mt-1 text-sm text-slate-500">{card.detail}</p> : null}</div>{card.phone ? <a href={`tel:${card.phone.replace(/[^+\d]/g, "")}`} className="shrink-0 rounded-xl border px-3 py-3 text-sm">Appeler</a> : null}{card.kind === "task" ? <button disabled={busy} className="shrink-0 rounded-xl border px-3 py-3 text-sm" onClick={() => void submit(`Marque la tâche « ${card.title} » comme faite.`, undefined, card.id)}>Terminer</button> : null}</div>)}{message.reply.choices && index === messages.length - 1 ? <div className="grid gap-2">{message.reply.choices.map(choice => <button disabled={busy} type="button" key={choice.id} onClick={() => void submit(choice.label, choice.id)} className="min-h-12 rounded-xl border border-blue-300 px-4 py-3 text-left">{choice.label}</button>)}</div> : null}{message.reply.draft ? <EmailDraft draft={message.reply.draft} /> : null}</div> : null}</div>)}
...
function EmailDraft({ draft }: { draft: NonNullable<CoachReply["draft"]> }) {
  const [subject, setSubject] = useState(draft.subject);
  const [message, setMessage] = useState(draft.message);
  const [editing, setEditing] = useState(false);
  const [notice, setNotice] = useState("");
  return <div className="space-y-3 rounded-2xl border border-slate-200 p-4"><p className="text-sm">Destinataire : {draft.recipient}</p><label className="block text-sm">Objet<input aria-label="Objet du brouillon" readOnly={!editing} value={subject} onChange={event => setSubject(event.target.value)} className="mt-1 w-full rounded-lg border bg-transparent p-2" /></label><textarea aria-label="Message du brouillon" readOnly={!editing} value={message} onChange={event => setMessage(event.target.value)} rows={8} className="w-full rounded-xl border bg-transparent p-3" /><div className="flex gap-2"><button type="button" onClick={() => setEditing(value => !value)} className="min-h-11 rounded-xl border px-4">{editing ? "Terminer" : "Modifier"}</button><button type="button" className="min-h-11 rounded-xl border px-4" onClick={() => { void navigator.clipboard.writeText(`À : ${draft.recipient}\nObjet : ${subject}\n\n${message}`).then(() => setNotice("Copié.")).catch(() => setNotice("Copie indisponible. Sélectionne le texte manuellement.")); }}>Copier</button></div><p role="status" className="text-xs text-slate-500">{notice || "Brouillon uniquement — aucun envoi."}</p></div>;
}
